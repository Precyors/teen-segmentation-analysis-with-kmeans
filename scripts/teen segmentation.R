...
# Cluster sizes
teen_clusters$size
# Create dataframe for cluster sizes
cluster_sizes <- data.frame(
  Cluster = as.factor(1:5),
...
ggplot(cluster_sizes,
  ...
  labs(
    title = "Cluster Sizes in Teen Market Segmentation",
    x = "Cluster",
    y = "Number of Teens"
  ) +
  theme_minimal()
...
teen_pca <- prcomp(interests_z)

# Create dataframe with first 2 principal components
pca_data <- data.frame(
  PC1 = teen_pca$x[,1],
  PC2 = teen_pca$x[,2],
  Cluster = as.factor(teen_clusters$cluster)
)
...
ggplot(pca_data,
  ...
  labs(
    title = "Teen Market Segmentation using K-Means",
